...
- After a shared font change, `WindowManager` calls `App::onUiScaleChanged()` so apps with cached layout, such as an open Filesystem context menu, can rebuild their hit geometry without pretending the window itself was resized.
