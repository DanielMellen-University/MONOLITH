...
- Filesystem Browser, Text Editor, Drawing, Pong, Breakout, Snake, and Minesweeper cache repeated SDL_ttf surfaces for toolbar labels, paths, syntax spans, line numbers, rows, status text, HUDs, overlays, and repeated glyphs; the caches are renderer-independent and clear when the shared UI scale changes. Text Editor also clears its cache when document content or the loaded document changes, while per-frame textures remain short-lived.
