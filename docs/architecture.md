...
- Screen-space taskbar, clock, and Start-menu hit targets are rebuilt during render and invalidated whenever desktop geometry, display scale, header offset, or interface font metrics change, so an event before the next frame cannot use coordinates from the previous layout.
