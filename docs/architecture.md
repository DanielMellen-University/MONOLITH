...
**Close note:** Before destroying a window, the shell calls `App::allowClose()`. Apps may return false once to warn about unsaved work (second close discards). The close path rechecks target identity after `allowClose()` and the focused app's `onFocusLost()` callback, so a callback that closes its own window cannot leave an outer close operation using a destroyed target. Start menu Shut Down and the host window's `SDL_QUIT` event both call the same contract on every open app in each request from a live window snapshot, so callback-triggered window changes cannot invalidate the validation pass and multiple dirty documents are warned together before the process can exit. Default is always allow.
