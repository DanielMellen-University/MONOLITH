...
- Filesystem Browser, Text Editor, Terminal, Settings, Drawing, Pong, Breakout, Snake, and Minesweeper cache repeated SDL_ttf surfaces for toolbar labels, paths, syntax spans, line numbers, scrollback rows, status text, HUDs, overlays, option text, information lines, and repeated glyphs; the caches are renderer-independent and clear when the shared UI scale changes. Text Editor rasterizes only viewport-intersecting UTF-8 syntax-span slices and clears its cache when document content, scroll position, client size, status feedback, or interface scale changes, and while dynamic find/replace or path-prompt text is rendered; Drawing clears status changes and dynamic path-prompt text similarly; Settings clears path changes and dynamic wallpaper prompt text; Terminal bounds scrollback to 2,000 rows and 8 MiB (64 KiB per row), command history to 500 entries and 2 MiB (64 KiB per entry), rasterizes only viewport-fitting UTF-8 prefixes, seeks to a bounded history-file tail before streaming it and caps oversized-record recovery at 16 MiB, and clears scrollback surfaces when output, scroll position, client size, or scale changes. Browser clears its cache on listing or status changes. Per-frame textures remain short-lived.
