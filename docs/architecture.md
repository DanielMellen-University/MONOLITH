...
- Filesystem Browser, Text Editor, Terminal, Settings, and Drawing cache repeated SDL_ttf surfaces for toolbar labels, paths, syntax spans, line numbers, scrollback rows, status text, option text, and information lines. These caches are renderer-independent and clear when their content or the shared UI scale changes; per-frame renderer textures stay short-lived. Text Editor rasterizes only viewport-intersecting UTF-8 syntax-span slices; Terminal bounds scrollback to 2,000 rows and 8 MiB (64 KiB per row), command history to 500 entries and 2 MiB (64 KiB per entry), rasterizes only viewport-fitting UTF-8 prefixes, seeks to a bounded history-file tail before streaming it and caps oversized-record recovery at 16 MiB. Browser clears its cache on listing or status changes; Drawing and Settings clear dynamic status and prompt text so old variants do not accumulate.
