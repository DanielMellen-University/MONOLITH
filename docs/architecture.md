...
The Window Manager broadcasts virtual path creation, change, move, and removal events to every open app. Filesystem Browser uses creation and change events to refresh a visible parent directory, while Terminal, Text Editor, and Drawing use move/remove events to keep working directories and file bindings coherent. Change events also invalidate the cached wallpaper when the changed path contains the active wallpaper. The broadcast is sent only after the filesystem operation succeeds.
