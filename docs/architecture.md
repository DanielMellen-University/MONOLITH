...
- App rendering is clipped to the window's client rectangle at the shell boundary, so tiny or undersized app layouts cannot paint into title bars or the taskbar. Apps that use narrower internal clips must intersect and restore the caller clip; Browser and Settings follow this rule explicitly. Client rectangles may be zero-sized on an undersized desktop, but are never negative.
