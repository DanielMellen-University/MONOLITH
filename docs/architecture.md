...
**Close note:** Before destroying a window, the shell calls `App::allowClose()`. Apps may return false once to warn about unsaved work (second close discards). Start menu Shut Down and the host window's `SDL_QUIT` event both call the same contract on every open app in each request from a live window snapshot, so callback-triggered window changes cannot invalidate the validation pass and multiple dirty documents are warned together before the process can exit. Default is always allow.
