...
After a successful `mkdir`, `touch`, or `cp`, open Filesystem Browser windows refresh when the new entry belongs directly to the folder they are viewing. Creating a file by saving a new Text Editor document or Drawing sketch uses the same notification path.
