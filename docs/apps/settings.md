...
- Cached swatch, clock, text-size, and wallpaper-control hit rectangles are cleared immediately when the client resizes or the interface scale changes, then rebuilt on the next render so a pre-render event cannot use the old layout.
