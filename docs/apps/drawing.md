...
- Cached toolbar button and color-swatch hit rectangles are cleared immediately on resize or interface-scale changes, then rebuilt on the next render so an event between frames cannot activate a control at its old position.
