...
Headless Terminal filesystem command test for empty directories, regular files, missing paths, root completion, and scrollback bounds:
...
Headless Filesystem Browser state test for filtering, multi-selection restoration, scaled chrome bands, status-bar hit testing, tiny-client row bounds, resize and scale scroll clamping, delete confirmation, and partial cut/paste:
...
Headless Text Editor state test for Save As collisions, failed-write recovery, font-scaled status geometry, and resize scroll bounds:
