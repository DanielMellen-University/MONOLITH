...
Headless Text Editor state test for Save As collisions, creation-notification binding order, failed-write recovery, font-scaled status geometry, complete-row mouse hit testing, and resize scroll bounds:
...
Headless WindowManager test that failed Editor and Drawing opens do not reserve stale file singletons, can be retried, skip stale file-backed session entries, and release bare-app instance slots when they become file-backed:
...
Headless test of WindowManager client mouse capture across focus changes, pointer exit, and host focus-loss recovery:
...
Headless test of keyboard focus handoff when the active window is minimized, when a minimized file singleton is reopened, and when focus callbacks close windows during activation or Start-menu suspension:
