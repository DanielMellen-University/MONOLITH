...
Headless test of keyboard focus handoff when the active window is minimized, when a minimized file singleton is reopened, and when a focus callback closes its own window:
...
The same lifecycle test also covers virtual-path notification dispatch and resize callbacks, including apps closing themselves while the shell broadcasts an event or reapplies desktop geometry.
