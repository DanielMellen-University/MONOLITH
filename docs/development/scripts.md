...
The same lifecycle test also covers virtual-path notification dispatch, resize callbacks, session restore geometry, and click activation, including apps closing themselves while the shell broadcasts an event, reapplies desktop geometry, restores a session entry, or finishes focusing an input target.
